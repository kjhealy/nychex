...
## BX1003, BX1071 are true geographic islands
## BX0291 (North & South Brother Islands) disconnects after simplification
## QN0151 (Rikers Island) is in the Bronx group despite the QN prefix
...
## Get hex spacing from the generated tiles
...
## BX1003 (City Island area) — east of the main cluster, mid-height
bx1003_pos <- c(
  bx_bbox["xmax"] + hex_spacing,
  mean(c(bx_bbox["ymin"], bx_bbox["ymax"]))
)

## BX1071 (Pelham Bay Park islands) — east of main cluster, upper
bx1071_pos <- c(
  bx_bbox["xmax"] + hex_spacing,
  mean(c(bx_bbox["ymin"], bx_bbox["ymax"])) + 2 * hex_spacing
)

## BX0291 (North & South Brother Islands) — south of main cluster
bx0291_pos <- c(
  bx_bbox["xmin"] + 2 * hex_spacing,
  bx_bbox["ymin"] - hex_spacing
)

## QN0151 (Rikers Island) — south-southeast of main cluster
qn0151_pos <- c(
  bx_bbox["xmin"] + 4 * hex_spacing,
  bx_bbox["ymin"] - hex_spacing
)
...
bx_hex_sf <- bx_main_sf |>
  ...
  bind_rows(
    bx_islands_sf |>
      ...
      select(-area) |>
      mutate(
        tile_map = do.call(c, island_tiles)
      ) |>
      st_as_sf(sf_column_name = "tile_map")
  )

## Save intermediate result
saveRDS(bx_hex_sf, here::here("data-raw", "hex_bronx.rds"))
...
p <- ggplot(bx_hex_sf) +
  ...
  labs(title = "Bronx NTA 2020 Hex Map") +
  theme_void()

ggsave(
  here::here("data-raw", "sample-figures", "bronx_hex.png"),
  p,
  width = 8,
  height = 8,
  dpi = 150,
