...
## MN0191 — south of the southern tip
south_tiles <- hex_coords[hex_coords[, 2] == min(hex_coords[, 2]), , drop = FALSE]
mn0191_pos <- c(
  mean(south_tiles[, 1]),
  min(hex_coords[, 2]) - 1.5 * hex_spacing
)

## MN1191 — northeast of the northern tip
mn1191_pos <- c(
...
saveRDS(mn_sq_sf, here::here("data-raw", "sq_manhattan.rds"))
...
p <- ggplot(mn_sq_sf) +
  ...
  labs(title = "Manhattan NTA 2020 Square Map") +
  theme_void()

ggsave(
  here::here("data-raw", "sample-figures", "manhattan_sq.png"),
  p, width = 6, height = 10, dpi = 150, bg = "white"
)
