...
## Single-hex islands (Jamaica Bay area)
bkqn_single_islands <- c("BK1891", "BK5691", "BK5692", "QN8491")

## Rockaway chain (4 contiguous hexes)
bkqn_rockaway <- c("QN1401", "QN1402", "QN1403", "QN8492")

## Disconnected after simplification
bkqn_disconnected <- c("QN0761", "QN1491")

## All NTAs to remove before main hex generation
...
##   horizontal neighbor offset: (sqrt(3) * s, 0)
##   diagonal neighbor offset: (sqrt(3)/2 * s, 1.5 * s)
## where s is hex_spacing / sqrt(3) (the inradius)

## Place Jamaica Bay islands in a row south of the main cluster
south_y <- bkqn_bbox["ymin"] - 1.5 * hex_spacing
south_x_center <- mean(c(bkqn_bbox["xmin"], bkqn_bbox["xmax"]))

## BK5692 (Jamaica Bay West) — left
bk5692_pos <- c(south_x_center - hex_spacing, south_y)
## QN8491 (Jamaica Bay East) — right
qn8491_pos <- c(south_x_center + hex_spacing, south_y)
## BK5691 (Barren Island-Floyd Bennett Field) — between, lower
bk5691_pos <- c(south_x_center - hex_spacing / 2, south_y - hex_spacing * 0.866)
## BK1891 (Marine Park-Plumb Island) — left of BK5691
bk1891_pos <- c(south_x_center - hex_spacing * 1.5, south_y - hex_spacing * 0.866)

## QN0761 (Fort Totten) — northeast corner of Queens
qn0761_pos <- c(bkqn_bbox["xmax"] + hex_spacing, bkqn_bbox["ymax"])

## QN1491 (Rockaway Community Park) — south, near Rockaways
qn1491_pos <- c(south_x_center, south_y - hex_spacing * 0.866)
...
## -- Position Rockaway chain (4 contiguous hexes, east-west) ----------------
## Anchor the chain south of the Jamaica Bay islands
rockaway_y <- south_y - hex_spacing * 0.866 * 2
rockaway_x_start <- south_x_center - hex_spacing * 1.5

## For pointy-topped hexes, horizontal neighbors are at x + hex_spacing
...
cat("Total BK/QN hexes:", nrow(bkqn_hex_sf), "\n")

## Save intermediate result
saveRDS(bkqn_hex_sf, here::here("data-raw", "hex_brooklyn_queens.rds"))

## Save sample figure
p <- ggplot(bkqn_hex_sf) +
  geom_sf(
  ...
  labs(title = "Brooklyn/Queens NTA 2020 Hex Map") +
  theme_void()

ggsave(
  here::here("data-raw", "sample-figures", "brooklyn_queens_hex.png"),
  p,
  width = 12,
  height = 10,
  dpi = 150,
